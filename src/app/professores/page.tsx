...
import { listTeachableModulesAction } from '@/features/public-teacher-registration/actions';
...
// Página pública: nunca exige login (BR-013/BR-016). Fica fora de
// src/app/(app) de propósito, pra nunca herdar o layout autenticado —
// ver PUBLIC_PATHS em src/lib/supabase/middleware.ts.
export default async function ProfessoresPage() {
  const modules = await listTeachableModulesAction();
  ...
  return (
    <div className="flex min-h-svh items-center justify-center bg-muted/30 px-4 py-10">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-background p-6 shadow-lg">
        ...
        <PublicTeacherRegistrationFlow modules={modules} />
      </div>
    </div>
  );
}
