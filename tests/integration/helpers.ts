...
/**
 * Helpers para os testes de integração das regras críticas (seção 34
 * da spec). Rodam contra o projeto Supabase REAL configurado em
 * .env.local — por isso ficam fora de `tests/unit` e só executam via
 * `npm run test:integration`, nunca no `npm test` padrão nem no CI
 * (ver vitest.integration.config.ts e o guard em critical-rules.test.ts).
 *
...
/** Hash idêntico ao usado nas funções trilho_* (digest sha256 + hex, ver 20260807120300_trilho_functions.sql). */
...
export async function cleanupFixtures(admin: SupabaseClient<Database>, ids: TestFixtureIds): Promise<void> {
  if (ids.classIds.length > 0) {
    await admin.from('attendance_history').delete().in(
      'attendance_id',
      (await admin.from('attendance').select('id').in('class_id', ids.classIds)).data?.map((r) => r.id) ?? [],
    );
    await admin.from('attendance').delete().in('class_id', ids.classIds);
    await admin.from('attendance_sessions').delete().in('class_id', ids.classIds);
    await admin.from('classes').delete().in('id', ids.classIds);
  }
